...
const flickrFeed = new GetFeed(
  'flickrData',
  `https://api.flickr.com/services/rest/?method=flickr.people.getPhotos&api_key=${flickrConfig.access_token}&user_id=${flickrConfig.user.id}&format=json` +
    `&per_page=${12}&page=1&extras=date_upload,last_update&nojsoncallback=1`,
)
  .getFeed()
  .then(data => {
    let i = 0
    ...
    for (i; i < arr.photo.length; i += 1) {
      ...
      normalizedOutput.push({
        ...
        embed_url: '',
        type: 'image',
        site_url: `https://www.flickr.com/photos/${flickrConfig.user.username}/${d.id}`,
